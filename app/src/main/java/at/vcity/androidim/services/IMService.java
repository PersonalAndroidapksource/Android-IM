 ...
 * in the same process as the application.  The {@linkLocalServiceController}
 * and {@linkLocalServiceBinding} classes show how to interact with the
 * service.
 *
...
*/	
	...
	{       
		...
		notification = mBuilder.setContentIntent(contentIntent)
				.setContentText("New message from " + username + ": " + msg)
				...
				public void run() 
				{
					try {					
						...
						if (tmp != null) {
							...
							sendBroadcast(i);
							Log.d("IMService","friend list broadcast sent");
						
						if (tmp2 != null) {
							i2.putExtra(MessageInfo.MESSAGE_LIST, tmp2);
							sendBroadcast(i2);	
							Log.i("IMService","friend list broadcast sent");
						}
						}
						else {
							Log.i("IMService","friend list returned null");
						}
					}
					catch (Exception e) {
						e.printStackTrace();
	...
	{				
		...
		{			
			...
			Log.i("IMService", "TAKE_MESSAGE broadcast sent by im service");
		}	
	...
	@Override
	public void onDestroy() {
		Log.i("IMService","IMService is being destroyed");
		super.onDestroy();
	}
